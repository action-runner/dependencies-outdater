...
describe("Given a node js workspace provider", () => {
  ...
  test("Should return a list of updates", async () => {
    ...
    const parsed: PackageFile = JSON.parse(
      nodejsProvider.updateSuggestions[0].content
    );

    expect(parsed.devDependencies!.mock_dev_dep).toBeDefined();
    expect(parsed.dependencies!.mock_dev_dep).toBeUndefined();

    expect(parsed.dependencies!.mock_dep).toBeDefined();
    expect(parsed.devDependencies!.mock_dep).toBeUndefined();
  });
